...
class DGCNN_cls(nn.Module):
    def __init__(self, k=20, emb_dims=1024, dropout=0.5):
        ...
        self.linear3 = nn.Linear(256, 2)  # Binary classification
    ...
    def forward(self, x):
        ...
        x = self.dp1(F.leaky_relu(self.bn6(self.linear1(x)), negative_slope=0.2))
        x = self.dp2(F.leaky_relu(self.bn7(self.linear2(x)), negative_slope=0.2))
        x = self.linear3(x)
        return x
