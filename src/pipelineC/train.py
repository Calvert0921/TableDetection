...
def train(model, train_loader, criterion, optimizer, device):
    ...
    for points, cat_vec, point_labels in train_loader:
        ...
        # Transpose output to match CrossEntropyLoss expected format [B, 1024, 2]
        outputs = outputs.permute(0, 2, 1)
        
        # Calculate loss - evaluate predictions for each point
...
def evaluate(model, val_loader, criterion, device):
    ...
    with torch.no_grad():
        for points, cat_vec, point_labels in val_loader:
            ...
            # Transpose output to match CrossEntropyLoss expected format
...
# Or use Focal Loss to handle extremely imbalanced data
...
if __name__ == "__main__":
    ...
    # Add class weights to reflect data imbalance
    class_weights = torch.tensor([1.0, 10.0], device=device)  # Background:Table = 1:10
