...
class DGCNN_seg(nn.Module):
    def __init__(self, k=20, emb_dims=1024, num_classes=2, dropout=0.2, categorical_dim=1):
        ...
        # Encoder part - EdgeConv layers same as classification model
        self.conv1 = nn.Sequential(nn.Conv2d(6, 64, kernel_size=1, bias=False),
                           nn.BatchNorm2d(64),
                           nn.LeakyReLU(negative_slope=0.2))
        ...
        # Decoder part - Propagate global features to each point
        ...
        # Segmentation head - Predict class for each point
        self.conv8 = nn.Conv1d(256, num_classes, kernel_size=1, bias=True)

        # MLP for processing categorical vector
    ...
    def forward(self, x, categorical_vector):
        ...
        # Process and expand categorical vector
        cat_vec = self.cat_mlp(categorical_vector).unsqueeze(2).expand(-1, -1, num_points)  # (B, 64, N)
    
        # Concatenate global features and categorical vector
        ...
        # Decoder - Propagate global features to each point
...
def test_forward():
    ...
    # Randomly generate a batch of point cloud data with 1024 points of dimension 3
    x = torch.randn(2, 1024, 3)
    # Randomly generate corresponding category vectors
    cat = torch.randn(2, 2)
    output = model(x, cat)
