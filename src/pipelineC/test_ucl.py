...
class CustomPointCloudDataset(Dataset):
    ...
    def __init__(self, npz_path):
        ...
        # Create default category vector (required by model)
    ...
    def __getitem__(self, idx):
        # Return point cloud, category vector, filename and frame index
...
def visualize_prediction(points, point_preds, save_dir, file_prefix, idx):
    ...
    has_table = table_ratio > THRESHOLD  # Using 1% threshold
...
if __name__ == "__main__":
    ...
    with open(results_file, 'w') as f:
        # Column headers matching content being written
        ...
        for i, (points, preds, filename, frame_idx) in enumerate(zip(dataset.pointclouds, point_preds, table_names, frame_indices)):
            ...
            # Determine table presence using 1% threshold
            has_table = table_ratio > THRESHOLD  
            
            # Use original filename as visualization file prefix
            file_prefix = f"{i:03d}_{clean_filename.split('.')[0]}"
            
            # Visualize
            visualize_prediction(points, preds, args.vis_dir, file_prefix, i)
            
            # Write results, including both surface ratio and binary outcome
