...
def build_dataset(base_dir, output_file):
    ...
    all_categorical_vectors = []  # scene type vectors
    ...
    for seq_idx, seq in enumerate(sequences):
        ...
        for i in tqdm(range(n_frames)):
            ...
            # Create category vector - contains scene type and table presence
            # Use one-hot encoding for scenes, [0,0,0,0,1] represents the 5th scene type
            # scene_type = np.zeros(len(sequences), dtype=np.float32)
            # scene_type[seq_idx] = 1.0
            # Add table presence flag
            ...
            if has_table:
                ...
                # Visualize original image and mask image
